  ...
  try {
    ...
    if (fillYourDetailsId) {
      ...
      if (!fillYourDetails) {
        return res
          .status(400)
          .json({ error: "FillYourDetails record not found" });
      }
    }
            ...
            (page: {
              ...
              components: {
                create: page.components.map((component) => {
                  const cleanedImageUrl = component.image_url
                    ? stripQueryParams(component.image_url)
                    : component.image_url;
                  return {
                    type: component.type,
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    try {
      ...
      if (!order) {
        return res
          .status(404)
          .json({ message: "No order found for this project" });
      }
...
app.post(
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    if (!projectId || !userId || !Array.isArray(layouts)) {
      return res
        .status(400)
        .json({ error: "Missing projectId, userId, or invalid layouts" });
    }
...
app.get(
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    if (!projectId || typeof projectId !== "string") {
      return res
        .status(400)
        .json({ message: "Invalid projectId or unauthorized" });
    }
      ...
      const pages = await Promise.all(
        contributionsData.map(async (contrib) => {
          const contributorName =
            contrib.fillYourDetails?.first_name || "Anonymous";
          const excludedFromBook = false;

          if (excludedFromBook) return [];

          return await Promise.all(
            contrib.pages.map(async (page) => {
              const components = await Promise.all(
                page.components.map(async (comp) => {
                  if (comp.type === "photo" && comp.imageUrl) {
                    ...
                    try {
                      const keyMatch =
                        cleanedImageUrl.match(/memorylane\/(.+)$/);
...
app.get(
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    try {
      ...
      if (
        frontCover?.config &&
        typeof frontCover.config === "object" &&
        "imageKey" in frontCover.config
      ) {
        const cleanedImageUrl = stripQueryParams(
          frontCover.config.imageKey as string
        );
        try {
          ...
          frontCoverImageUrl = await getSignedUrl(s3, command, {
            expiresIn: 3600,
          });
        } catch (error) {
      ...
      const html = generateBookHtml({
        ...
        frontCover: frontCover
          ? {
              ...
              descriptionStyle:
                (frontCover.config as any)?.descriptionStyle || {},
      ...
      const config = {
        ...
        json: {
          user_credentials:
            process.env.DOCRAPTER_API_KEY! || "3jOFcaZidoZMhyOjTQ_Y",
          doc: {
            ...
            prince_options: {
              ...
              baseurl:
                ...
                "https://memorylane.db134517dd79f4a26d091b4dcda7e499.r2.cloudflarestorage.com",
            },
          },
        },
      };
      ...
      request.post(config, (err, response, body) => {
        ...
        if (response.statusCode !== 200) {
          ...
          return res
            ...
            .json({ error: "DocRaptor API error" });
        }
    ...
    try {
      ...
      const deadline =
        project.deadlines.length > 0
          ? project.deadlines[0].actual_deadline ??
            project.deadlines[0].calculate_date
          : null;
...
app.patch(
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    if (!deadlineDate || typeof deadlineDate !== "string") {
      return res
        .status(400)
        .json({ message: "Invalid or missing deadlineDate" });
    }
    ...
    if (parsedDate < now) {
      return res
        .status(400)
        .json({ message: "deadlineDate must be in the future" });
    }
    ...
    try {
      ...
      const deadline =
        (updatedProject?.deadlines?.length ?? 0) > 0
          ? updatedProject?.deadlines?.[0]?.actual_deadline ??
            updatedProject?.deadlines?.[0]?.calculate_date
          : null;
...
app.patch(
  "/api/update-contribution/:contributionId",
  async (req: Request, res: Response): Promise<any> => {
    try {
      ...
      if (!contributionId || !fillYourDetailsId) {
        return res
          .status(400)
          .json({ error: "Missing contributionId or fillYourDetailsId" });
      }
      ...
      if (contribution.fillYourDetailsId) {
        return res
          .status(400)
          .json({ error: "Contribution already linked to FillYourDetails" });
      }
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    try {
      ...
      if (!shipping_address || !shipping_address.postal_code) {
        return res
          .status(400)
          .json({ error: "Shipping address with postal code is required" });
      }
      ...
      const shippingOptions =
        shiprocketResponse.data.data.available_courier_companies.map(
          (courier: Courier) => ({
    ...
    try {
      ...
      if (!project_id || !amount) {
        return res
          ...
          .json({
            error:
              "Missing required fields: project_id and amount are required",
          });
      }
...
app.post(
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Missing or invalid projectId or order_id" });
    }
    ...
    try {
      ...
      const luluTokenResponse = await axios.post(
      ...
      formData.append(
        "file",
        new Blob([Buffer.from(pdfResponse.data)]),
        `book-${projectId}.pdf`
      );

      const uploadResponse = await axios.post(
        "https://api.lulu.com/files/",
        formData,
        {
          headers: {
